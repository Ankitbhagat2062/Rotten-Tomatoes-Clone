...
let currentIndex = 0; // Move currentIndex outside the function to maintain its state
...
function showMoreEpisode() {
    ...
    EpisodesList.forEach(list => {
        ...
        const nextCards = hiddenCards.slice(currentIndex, currentIndex + cardsToShow);
        ...
        currentIndex += nextCards.length; // Increment by the number of cards shown

        // Check if there are remaining hidden cards
        ...
        if (currentIndex >= hiddenCards.length) {
            list.querySelector('.showMoreEpisode-down').style.display = 'none';
        } else {
...
const initializeVideoModal = () => {
    ...
    document.querySelectorAll(".movie-card").forEach(card => {
        ...
        card.addEventListener('click', async (e) => {
            ...
            const sortedMovies = Object.keys(Data)
                .flatMap(category => Data[category])
                .filter(movie => movie.name !== movieName && getMatchScore(movie) > 0)
        ...
        <div class="p-4">
            ...
            const createEpiosdeAlbumHTML = (movieData) => {
                if (movieData.name.includes("Season")) {
                    const season =
                        `<div style="position: relative;" class=" playlist bg-black text-white font-sans relative">
            ...
            <div class="p-6 bg-black text-white">
                ...
                <div class="mb-8">
                    ...
                    const season =
                        `<div data-video="${movieData.video}"  style="position: relative;" class="playlist bg-black text-white font-sans relative">
                    <div class="header-section">
                        <video class="background-video" muted loop>
                            <source
            ...
            const renderPlaylistSections = (categories) => {
                const movieAlbumContainer = document.querySelector("main");
            ...
            document.querySelectorAll('.play-button').forEach((playButton) => {
                if (playButton) {
                    playButton.addEventListener('click', async (e) => {
                        const videoUrl = e.currentTarget.closest('.movie').getAttribute('data-video');
    ...
    searchInput.addEventListener("input", () => {
        ...
        document.querySelectorAll(".movie-card").forEach(card => {
            if (card.querySelector(".title").textContent.toLowerCase().includes(searchTerm)) {
                ...
                clonedCard.addEventListener('click', async (e) => {
                    const movieName = movieInfoElement.querySelector('.title').textContent
                    let Data = await fetch('./movie.json').then(res => res.json())
